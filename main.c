...
{
   printf("========================================\n");
   printf("GLOBAL SUM PROBLEM - PRECISION ANALYSIS\n");
   printf("Testing different summation algorithms\n");
   printf("========================================\n\n");
   ...
   for (int pow_of_two = 10; pow_of_two <= 27; pow_of_two++) {
      ...
      // Test all summation methods
      struct SumMethod {
         char name[30];
         double (*function)(double*, long);
      };
      
      struct SumMethod methods[] = {
         {"Standard double sum", do_sum},
         {"Pairwise summation", do_pair_sum},
         {"Kahan summation", do_kahan_sum},
         {"Knuth summation", do_knuth_sum}
      };
      ...
      for (int i = 0; i < num_methods; i++) {
         ...
         test_sum = methods[i].function(energy, ncells);
         cpu_time = cpu_timer_stop(cpu_timer);

         printf("  accurate sum %-17.16lg\n", accurate_sum);
         printf("  computed sum %-17.16lg\n", test_sum);
         printf("  absolute diff %10.4le\n", test_sum - accurate_sum);
         printf("  relative diff %10.4le\n", (test_sum - accurate_sum) / accurate_sum);
         printf("  runtime %8.6lf seconds\n", cpu_time);
         printf("  Method: %s\n\n", methods[i].name);
      }
      
      // Test long double separately
      printf("  LONG DOUBLE PRECISION:\n");
      struct timespec cpu_timer;
      double cpu_time;
      
      cpu_timer_start(&cpu_timer);
      long double longdouble_sum = do_longdouble_sum(energy, ncells);
      cpu_time = cpu_timer_stop(cpu_timer);
      
      printf("  accurate sum %-17.16lg\n", accurate_sum);
      printf("  computed sum %-17.16Lg\n", longdouble_sum);
      printf("  absolute diff %10.4Le\n", longdouble_sum - (long double)accurate_sum);
      printf("  relative diff %10.4Le\n", (longdouble_sum - (long double)accurate_sum) / (long double)accurate_sum);
      printf("  runtime %8.6lf seconds\n", cpu_time);
      printf("  Method: Long double precision\n\n");
