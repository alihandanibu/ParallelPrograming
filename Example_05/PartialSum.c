...
int main(int argc, char *argv[])
{
    ...
    int nsize = base + (rank < remainder ? 1 : 0);
    ...
    // MPI_Scatterv sends variable-sized portions of the global array from the root process
    // to all other processes, based on nsizes[] and offsets[].
    // Each process receives exactly its assigned chunk.
    ...
    // Only rank 0 allocated the global array (a_global),
    // so only it should free that memory to avoid invalid free calls.
